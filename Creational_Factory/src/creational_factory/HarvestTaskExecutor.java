...
public class HarvestTaskExecutor implements CropTaskExecutor{
     public void executeTask(String crop) throws SQLException {
        ...
        try (Connection connection = AgriculturalDatabaseConnection.getInstance().getConnection();
             PreparedStatement stmt = connection.prepareStatement("UPDATE crops SET last_harvested = NOW() WHERE name = ?")) {
            stmt.setString(1, crop);
            stmt.executeUpdate();
            System.out.println("Harvested crop: " + crop);
        }
    }
}